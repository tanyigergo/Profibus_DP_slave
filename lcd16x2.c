...
void lcdPrintInt(uint8_t value)
{
	char str[16];
	sprintf(str, "%d", value);
	lcd16x2_puts(str);
...
/**
  ******************************************************************************
	* @brief	Prints integer value in hc format, 
						also prints 0x: in front of the number
  * @param	integer value
  * @retval	None
  ******************************************************************************
  */

void lcdPrintHex(uint8_t value)
{
	...
	upperNibble &= (~(0xF0));
	uint8_t lowerNibble = value & (~(0xF0));
	sprintf(cUpperNibble, "%X", upperNibble);
	sprintf(cLowerNibble, "%X", lowerNibble);
	
	//kiiratás
	
	lcd16x2_puts(cUpperNibble);
	lcd16x2_puts(cLowerNibble);
}
